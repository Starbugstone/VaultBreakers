...
for policy, name in enumerate(['MeleeFirst', 'RangedPressure', 'GuardApproach', 'Evasive', 'Mixed']):
    ...
    return_code = 0
    if not collect_only:
        return_code = subprocess.run([sys.executable, str(root/'Tools/Unity/review.py'), 'journey',
                                     '1920', '1080', label, '120', '--poc-policy', str(policy)]).returncode
    ...
    passed = return_code == 0 and session.get('completed') and session.get('replayReset') and session.get('shotsWhileGuardRaised') == 0
