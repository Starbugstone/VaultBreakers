...
namespace Vaultbreakers.Tests.PlayMode
{
    public sealed class DungeonPlayModeTests
    {
        ...
        [UnityTest] public IEnumerator GateRequiresClearAndWalkingAdvancesAllRoomsToRewardAndReplay()
        {
            for(var room=0;room<3;room++)
            {
                ...
                if(room<2)
                {
                    ...
                    Move(new Vector3(0,0,zone.RoomOrigin.z+14.1f));yield return null;
                }
            }
            Assert.That(zone.State,Is.EqualTo(ZoneState.Complete));Move(new Vector3(0,0,44.1f));yield return null;
        ...
        [UnityTest] public IEnumerator DeathRetriesCurrentRoomAndClearsAllActiveShots()
        {
            ...
            Assert.That(zone.Player.transform.position.z,Is.EqualTo(16.1f).Within(.2f));
        }
        [UnityTest] public IEnumerator PropsBlockThePlayerAndInitialSpawnsAreSeparated()
        {
            ...
            Move(new Vector3(5.6f,0,1));zone.Player.GetComponent<CharacterController>().Move(Vector3.forward*2);
            Assert.That(zone.Player.transform.position.z,Is.LessThan(1.7f),"The visible crate at (5.6,2.1) must block movement.");
            Clear();yield return null;Move(new Vector3(1.96f,0,6));zone.Player.GetComponent<CharacterController>().Move(Vector3.forward*2);
            Assert.That(zone.Player.transform.position.z,Is.LessThan(6.7f),"An open gate's solid posts must still block movement.");
        ...
        [UnityTest] public IEnumerator ClosedExitBlocksMovementAndCombatCameraDoesNotTrackPlayer()
        {
            yield return Fight();yield return new WaitForSeconds(.3f);var camera=Camera.main;var before=camera.transform.position;
            Move(new Vector3(0,0,6));var motor=zone.Player.GetComponent<CharacterController>();motor.Move(Vector3.forward*4);
            Assert.That(zone.Player.transform.position.z,Is.LessThan(7));
            Move(new Vector3(3,0,2));yield return new WaitForSeconds(.2f);
            Assert.That(Vector3.Distance(before,camera.transform.position),Is.LessThan(.05f));
        }
    }
}
